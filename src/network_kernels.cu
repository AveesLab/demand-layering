...
//    printf("%s\n",weights);
    ...
    int buf_size = (max_size_of_n + max_size_of_nweights)*2;
...
//    float attach_time = get_time_point();
//    cudaStreamAttachMemAsync(get_cuda_stream(), hGlobal_layer_weights, 0, cudaMemAttachHost);
//    cudaStreamAttachMemAsync(get_cuda_stream(), hGlobal_layer_weights, 0, cudaMemAttachGlobal);
//    printf("attach time(G2H) %lf\n",(get_time_point()-attach_time)/1000);
...
//        printf("%d layer\n",i);
		...
		if (i!=0 && kernel_idx < net.n){
			if (cudaEventQuery(kernel[kernel_idx]) == cudaSuccess){
//				printf("kernel[%d] is over, size: %'zu", kernel_idx, n_size[kernel_idx]*sizeof(float));
				//after kernel execution is done, device buffer's head has to move
				head[head_idx] += n_size[kernel_idx];
				kernel_idx++;
				while(n_size[kernel_idx] == 0) kernel_idx++;
				if(head[head_idx] + n_size[kernel_idx] > buf_size){
					head_idx += 1;
				}
//				printf("  --->  head[%d]: %'zu\n", head_idx, head[head_idx]*sizeof(float));
			}
		}
        if(l->type == CONVOLUTIONAL)
        {
			...
			if((head_idx == tail_idx) && tail[tail_idx] + n_size[i] > buf_size) tail_idx++;
			if((head_idx == tail_idx) || (tail[tail_idx] + n_size[i] <= head[head_idx])){
                cudaStreamAttachMemAsync(get_cuda_stream(), hGlobal_layer_weights, 0, cudaMemAttachHost);
//                printf("after AttachHost %d\n",i);
            	read_bytes = fread(hGlobal_layer_weights + tail[tail_idx], sizeof(float), n_size[i], fp);
//                printf("READ\n");
            	l->biases_gpu = tail[tail_idx] + hGlobal_layer_weights;
            	l->weights_gpu = tail[tail_idx] + hGlobal_layer_weights + l->n;

#ifdef NVTX
                nvtx_forward_gpu = nvtxRangeStartW(L"l.forward_gpu");
#endif //NVTX
//                cudaStreamAttachMemAsync(get_cuda_stream(), hGlobal_layer_weights, 0, cudaMemAttachGlobal);
//                printf("after AttachGlobal %d\n",i);
        		//KERNEL CALL
...
//                int p = 0;
//                cudaDeviceGetAttribute(&p, cudaDevAttrConcurrentManagedAccess, 0);
//                if(p) printf("p = %d\n",p);

				tail[tail_idx] += n_size[i];
//				printf("%3d layer, size : %'d, tail[%d]: %'d, head[%d]: %'d\n",i,n_size[i]*4,tail_idx,tail[tail_idx]*4, head_idx, head[head_idx]*4);
			}
			else{
//				printf("%3d layer is not complited // available space %'d need space %'d\n",i,sizeof(float)*(head[head_idx]-tail[tail_idx]),n_size[i]*4);
				i--;
				cudaEventSynchronize(kernel[kernel_idx]);
//				printf("kernel[%d] is over, size: %'zu\n", kernel_idx, n_size[kernel_idx]*sizeof(float));
//				printf("head[%d] : %'d kernel[%d] : %'d\n",head_idx,head[head_idx]*sizeof(float), kernel_idx,n_size[kernel_idx]*sizeof(float));
			}
        }
		else{	//Non-Convolutional
#ifdef NVTX
            nvtx_forward_gpu = nvtxRangeStartW(L"l.forward_gpu");
#endif //NVTX
//			printf("%3d layer NON CONV\n",i);
        	//KERNEL CALL
	        l->forward_gpu(*l, state);
			cudaEventRecord(kernel[i]);
	        state.input = l->output_gpu;
...
//        cudaEventDestroy(kernel[i]);
    if(infer_count < 100) infer_count++;
    cudaDeviceSynchronize();
    printf("inference time: %lfms\n",(get_time_point() - cur_time)/1000);
    e_time[e_count] = (get_time_point() - cur_time)/1000.0;
    e_count++;
#endif
//    cudaDeviceSynchronize();
